  ...
  try {
    ...
    for (const monitor of monitors) {
      ...
      if (monitor.status !== status) {
        ...
        monitor.isAlertSent = status === "down"; // alert only on down
        await monitor.save();
        ...
        if (user && user.email) {
          ...
          const message = `Monitor: ${monitor.name}\nURL: ${
            monitor.url
          }\nStatus: ${status.toUpperCase()}\nChecked at: ${new Date().toLocaleString()}`;
          await sendEmail(user.email, subject, message);
        }

        // 🔔 Trigger alert here (email/telegram — coming later)
        console.log(
...
const startCron = () => {
  cron.schedule("* * * * *", checkUptime); // every 1 minute
